...
void bigMoneySub(char [], char [], int [], int, int );						//宣告大數減法函數 
...
{	
	...
	bigMoneySub(m1,m2,diff,a,b);											//執行bigMoneySub 
...
void bigMoneyAdd(char m1[], char m2[], int sum[],int a,int b)				//函數bigMoneyAdd 
{
	...
	for(int i=0; i<a; i++)													//把x,y相對應的位數加起來 
	{
		sum[i]=x[i]+y[i];
	}
	for(int i=0; i<a; i++)													//把每個位數都檢查一遍 
	{
   		if(sum[i]>=10) 														//如果有>10的 
		{
...
void bigMoneySub(char m1[], char m2[], int diff[], int a, int b)			//函數bigMoneyAdd
{
	int x[101]={0},y[101]={0};
	for(int i=0; i<a; i++)													//把m1的值的格式從char改成int
	{
	...
	for(int i=0; i<a; i++)													//把x減掉y相對應的位數
	{
		diff[i]=x[i]-y[i];
	}
	for(int i=0; i<a-1; i++)												//把每個位數都檢查一遍
	{
   		if(diff[i]<0) 														//如果有<0的
		{
     		diff[i+1]--;													//下一位數減1 
