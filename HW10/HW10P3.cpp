...
Table* Table::executeCmd(Command cmd) /* use your own code in homework 9 and MODIFY (Problem 3 and Problem 4)*/
{
	...
	if (strcmp(cmd.select[0], "*") == 0)									//看是不是* 
	{
		for (int i = 0; i < numOfField; i++)
		{
			ansTab->addField(field[i].name, field[i].isString);
		}
		...
		for (int j = 0; j < numOfEntry; j++)
		{
			...
			{
				if (field[k].isString)
					strcpy(entry[k].c, field[k].data[j].c);	
				else
				{
					entry[k].i = field[k].data[j].i;
//					cout << entry[k].i << "\n";
				}
			}
	...
	else
	{
		...
		for (int j = 0; j < numOfEntry; j++)
		{
			for (int k = 0; k < cmd.selectNum; k++)
			{
				...
				{
					if (tempFie->isString)
						strcpy(entry[k].c, tempFie->data[j].c);	
					else
					{
						entry[k].i = tempFie->data[j].i;
//						cout << entry[k].i << "\n";
					}
				}	
			}
